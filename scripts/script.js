...
var input = [
    ...
    { ratingRevievs: "403 отзыва", price: "4 349 грн", name: "Xiaomi Redmi Note 4X 3/32GB Black" }, { ratingRevievs: "488 отзывов", price: "6 199 грн", name: "Samsung Galaxy J7 (2016) J710F/DS Gold + защитное стекло + чехол!" },
    ...
    { ratingRevievs: "93 отзыва", price: "18 999 грн", name: "Apple iPhone 7 32GB Black" }, { ratingRevievs: "33 отзыва", price: "16 999 грн", name: "Huawei P10 4/32GB Black + сертификат 2500грн + чехол Huawei Smart View Cover!" },
    { ratingRevievs: "71 отзыв", price: { oldUan: "2 399 грн", newUan: "1 999 грн" }, name: "LG K5 X220ds Gold" }, { ratingRevievs: "39 отзывов", price: "2 995 грн", name: "Lenovo C2 Power (K10a40) Black" },
    { ratingRevievs: "156 отзывов", price: "2 599 грн", name: "Nous NS 5006 Gold" }, { ratingRevievs: "40 отзывов", price: "19 689 грн", name: "LG G6 Mystic White" },
    { ratingRevievs: "24 отзыва", price: "5 995 грн", name: "Motorola MOTO G5 (XT1676) Grey" }, { ratingRevievs: "7 отзывов", price: { oldUan: "10 999 грн", newUan: "9 999 грн" }, name: "HTC One X10 Dual Sim Silver" },
...
pOldPrice.className = "p-old-price";
...
console.log(typeof (input[0]["price"]));

for (let i = 0; i < input.length; i++) {
    let divClone = div.cloneNode(true);
    main.append(divClone);

    let pNameClone = pName.cloneNode(true);
    divClone.append(pNameClone);

    let pRatingClone = pRating.cloneNode(true);
    divClone.append(pRatingClone);

    let pOldPriceClone = pOldPrice.cloneNode(true);
    divClone.append(pOldPriceClone);

    let pPriceClone = pPrice.cloneNode(true);
    divClone.append(pPriceClone);

    pNameClone.append(input[i]["name"]);
    pRatingClone.append(input[i]["ratingRevievs"]);

    if (typeof (input[i]["price"]) === 'string') {
        pPriceClone.append(input[i]["price"]);
    }

    else {
        pOldPriceClone.append(input[i].price["oldUan"])
        pPriceClone.append(input[i].price["newUan"]);
        pPriceClone.className = "p-price p-new-price";
    }
}


function SelectSort() {
    var sel = parseInt(document.getElementById('select-sort').value);
    SortByPrice(sel);
    RenderPage();
}

var divList = document.getElementsByClassName("div-phones");
divList = Array.prototype.slice.call(divList);

function SortByPrice(desc) {
    divList.sort(function (a, b) {
        a = parseInt(a.lastChild.innerHTML.replace(/\s+/g, ''));
        b = parseInt(b.lastChild.innerHTML.replace(/\s+/g, ''));
        if (a > b) {
            return 1 * desc;
        }
        if (a < b) {
